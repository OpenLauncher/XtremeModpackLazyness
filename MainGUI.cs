		...
		private void button2_Click (object sender, EventArgs e)
		{
			...
			if (folderBrowserDialog1.ShowDialog () == DialogResult.OK) {
				...
				foreach (string stl in filePaths) {
					...
					if (customPackBox.Checked) {
						if (str.Contains ("forge")) {
							//Forge is never a custom mod so dont give it a custom mod
							file.Write ("<mod url='files/1710/" + str + "' file='" + str + "' download='server' name='" + result + "' type='forge' md5='" + GetHash (stl) + "'/>" + Environment.NewLine);
						} else {
							file.Write ("<mod url='files/1710/" + str + "' file='" + str + "' download='server' name='" + result + "' type='mods' md5='" + GetHash (stl) + "' customMod=\"yes\" />" + Environment.NewLine);
						}
					} else {
						if (str.Contains ("forge")) {
							file.Write ("<mod url='files/1710/" + str + "' file='" + str + "' download='server' name='" + result + "' type='forge' md5='" + GetHash (stl) + "'/>" + Environment.NewLine);
